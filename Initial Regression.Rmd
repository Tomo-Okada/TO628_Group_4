...
#KNN Model Results Confusion Matrix
confusionMatrix(as.factor(dssal_knn_test_pred1),as.factor(dssalariestest_norm$salaryabove),positive = "1")
...
#Stacke model results Confusion Matrix
...
### Machine Learning Model and Stacked Model results
We conducted other analysis using ML algorithms and constructed the second level stacked model. The ANN shows the highest Accuracy **0.7008** and the highest Kappa **0.5852** among individual models. However, the second level combined model could not improve Accuracy and Kappa which is **0.694** and **0.3559** respectively.
...
Ariel to add details on model tuning and years coding significance later this weekend
